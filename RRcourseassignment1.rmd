...
activity$interval2 <- as.POSIXct(strptime(activity$interval2,format="%H%M"))
```
...
max <- max(inter$inter)
when <- names(which.max(inter$inter))
```

The 5-minute interval, on average across all the days in the dataset, which contains the maximum number of steps (**`r max`**) is at the interval **`r when`**. 
...
We tried to use the R package [rrcovNA][1] to impute the missing values. Sounded fair. 

We used the function for conducting sequential nearest neighbor 
imputation ('impSeq'), which uses statistical measures of distance (mean & covariance) to impute missing values. There is a good explainer [available online][2]. 

Calling rrcovNA seems to show all the package loading. Sorry for the inconvenience in the presentation. If you know how I can mute the warnings, please do write to me and I'll submit another version.

```{r imputevals, echo=TRUE}
library(rrcovNA)
imputed.act <- as.data.frame(impSeq(activity))
imputed.act$date  <- as.Date(activity$date)
imputed.act$interval <- 
...
```{r comparative}
par(mfrow = c(1, 2))
hist(actdata,breaks=20,main="Total original steps",col="red",xlab="steps per day", ylab="# of days",ylim=c(0,15))

hist(actdata2,breaks=20,main="Total imputed steps",col="red",xlab="steps per day", ylab="# of days",ylim=c(0,15))
```

Doesn't seem to have significantly changed the patterns in the database in most days. In part this is good, because it seems not to introduce distortion in the overall patterns. But does it introduce distortion in each point? "Further studies may assess that", as they say in science. 


### Problem 5: Are there differences in activity patterns between weekdays and weekends?

To analyze that, we need to create a new factor variable in the dataset with two levels - "weekday" and "weekend" indicating whether a given date is a weekday or weekend day.
...
newImputed$interval2 <- activity$interval
...
Now, I need to make a panel plot containing a time series plot (i.e. type = "l") of the 5-minute interval (x-axis) and the average number of steps taken, averaged across all weekday days or weekend days (y-axis).

I must confess I had trouble fixing the data on the format I needed to get the right transform done. Lattice panels would not work with aggregate data in the way I was doing. Also, for some reason, a previous transform of intervals into hours of day was resulting in today's date plus the right hour. 

And then a colleague in the forums mentioned we could use Aggregate. So I did. The 20-plus lines I was using for quite a few days became just three. For this relief, much thanks to [Maher Harb][3]. 

```{r lattice}
inter2 <- aggregate(steps ~ interval2 + dayOfWeek , data = newImputed, mean)

library(lattice)
xyplot(steps~interval2|dayOfWeek,data=inter2,type="l")
```

So, I think we're done for the first assignment. Yay!


[1]: http://cran.r-project.org/web/packages/rrcovNA/index.html
[2]: http://www.unt.edu/rss/class/Jon/Benchmarks/MissingValueImputation_JDS_Nov2010.pdf
[3]: https://class.coursera.org/repdata-005/forum/thread?thread_id=72